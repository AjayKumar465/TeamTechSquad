...
SELECT DEFICIENCY.DEFICIENCY,DEFICIENCY_CATEGORY.CATEGORY  FROM DEFICIENCY JOIN 
DEFICIENCY_CATEGORY ON DEFICIENCY_CATEGORY.ID  = DEFICIENCY.DEFICIENCY_CATEGORY_ID;
 
SELECT ID,CATEGORY FROM DEFICIENCY_CATEGORY;
 
SELECT DEFICIENCY.ID,DEFICIENCY.DEFICIENCY FROM DEFICIENCY WHERE DEFICIENCY.DEFICIENCY_CATEGORY_ID  =?;
 
UPDATE USERS SET  EMAIL = ?, FIRST_NAME =?,LAST_NAME =?, MOBILE =? WHERE USER_ID  = ?	;
SELECT COUNT(*) FROM USERS WHERE (EMAIL = ?) OR (MOBILE = ?);
CREATE TABLE FOODSOURCE(ID INT(11) NOT NULL AUTO_INCREMENT,SOURCE_NAME VARCHAR(256) DEFAULT NULL);
CREATE TABLE VITAMIN_FOODSOURCE(ID INT(11) NOT NULL AUTO_INCREMENT,VITAMIN_ID  INT(11)  DEFAULT NULL, FOODSOURCE_ID INT(11) DEFAULT NULL);
